...
class HTML5 extends React.Component<any, any, any> {
  static propTypes = {
    ...vendorPropTypes,
  ...
  _bindAudioObjectEvents() {
    ...
    Object.keys(playerEvents).forEach(key => {
      this._player[key.toLowerCase()] = (playerEvents as any)[key];
    });
  }
  ...
  _handleProgress = ({ target: { buffered, duration } }: any) => {
    let progress = 0;
  ...
  _handleDuration = ({ target: { duration } }: any) => {
    this.props.onDuration(duration);
  };

  _handleTimeUpdate = ({ target: { currentTime } }: any) => {
  ...
  render() {
    ...
    return useAudioObject
      ? null
      : React.createElement(vendor, {
          ref: (c: any) => (this._player = c),
          src,
          ...playerProps,
          ...this._playerEvents,
        });
